...
# _log_file = open(os.path.join(_LOG_DIR, "run_logs_backend.txt"), "a", encoding="utf-8")
...
def register_routes(app, workflow):
    ...
    def evaluate():
        try:
            ...
            dataset_file = data.get("dataset_path", "").strip()

            if not dataset_file:
                mapped = dataset_map.get(pdf_name)
                if not mapped:
                    return jsonify({"error": f"No dataset mapping for {pdf_name}"}), 404
                dataset_file = os.path.join(os.path.dirname(__file__), "..", mapped)
                dataset_file = os.path.abspath(dataset_file)
...
#it workd nd final onne but for quac dataset to run stability runner need to change paths and all in evalution one so gonna chnage it above
#  import logging
# import os
# import json
# import time
...
# import os got some problemw ith running terminal one in backend.txt one 
# import json
# import time
# import uuid
# import sys
...
#         # reuse your existing /process logic
#         emit_event(request_id, "extract_start", "📄 Extracting PDF text...")
#         initial_state: DocState = {
#             "pdf_path":       pdf_path,
#             "question":       question,
...
#             emit_event(request_id, "done", "✅ Complete!")
#             cleanup_queue(request_id)
#             return jsonify({"answer": result["answer"], "metrics": m})
#         except Exception as e:
#             return jsonify({"answer": f"Error: {str(e)}", "metrics": {}})
